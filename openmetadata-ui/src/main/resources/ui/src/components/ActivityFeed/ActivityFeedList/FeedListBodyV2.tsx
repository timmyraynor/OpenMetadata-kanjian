...
import { getDateByTimeStamp, getTimeByTimeStamp } from 'utils/TimeUtils';
import {
  Post,
  Thread,
  ThreadTaskStatus,
...
const FeedListBodyV2: FC<FeedListBodyProp> = ({
  updatedFeedList,
  relativeDay,
  isEntityFeed,
  onThreadSelect,
  ...
  const getTimelineChild = (
    ...
    return (
      <Card
        ...
        key={`${index} - card`}
        onClick={() => feed.task && handleCardClick(feed.task.id, isTask)}>
        {isTask && <TaskBadge status={feed.task?.status as ThreadTaskStatus} />}
  ...
  const getTimelineLable = (feed: Thread) => {
    const time = feed.threadTs ? getTimeByTimeStamp(feed.threadTs) : 'unknown';
    const date = feed.threadTs ? getDateByTimeStamp(feed.threadTs) : 'unknown';

    return (
      <>
        <strong>{t('label.event-time')}</strong>
        <div>{time}</div>
        <div>{date}</div>
      </>
    );
